...
public class LoginPageTest extends BaseClass {
	...
	public void verifyValidLoginTest(String username, String password) {
		...
		BaseClass.logger.info("Verifying Admin tab is visible or not");
		Assert.assertTrue(homePage.isAdminTabVisible(),"Admin tab should be visible after successfull login ");
		ExtentManager.logStep("Validation Successful");
		BaseClass.logger.info("Login Successful with valid credentials!");
		homePage.logout();
		ExtentManager.logStep("Logged out Successfully!");
		BaseClass.logger.info("Logged out Successfully!");
		staticWait(2);
	}
	...
	public void inValidLoginTest(String username, String password) {
		...
		System.out.println("Running testMethod2 on thread: " + Thread.currentThread().getName());
		ExtentManager.logStep("Navigating to Login Page entering username and password");
		BaseClass.logger.info("Navigating to Login Page entering username and password");
		loginPage.login(username, password);
